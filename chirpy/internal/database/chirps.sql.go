...
const fetchChirpsByUserID = `-- name: FetchChirpsByUserID :many

SELECT id, created_at, updated_at, body, user_id FROM chirps WHERE user_id = $1 ORDER BY created_at ASC
`

func (q *Queries) FetchChirpsByUserID(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, fetchChirpsByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const fetchSingleChirp = `-- name: FetchSingleChirp :one

SELECT id, created_at, updated_at, body, user_id FROM chirps WHERE id = $1
`

func (q *Queries) FetchSingleChirp(ctx context.Context, id uuid.UUID) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, fetchSingleChirp, id)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
